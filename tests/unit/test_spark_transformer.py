...
class TestTransformDimContinent:
    ...
    def test_basic_transform(self, spark, transformer):
        raw = spark.createDataFrame(
            [("EU", "europe"), ("AM", "amerique")],
            schema=["CONTINENT_CD", "CONTINENT_NM"]
        )
...
class TestTransformFactTransactions:
    ...
    def _make_raw_txn(self, spark):
        return spark.createDataFrame(
            [
                ("TXN001", "20240101", "CUST001", "PROD001", "BUY",  "SETTLED", "ONLINE", 1100.0, 1000.0, 100.0, 0.0, 10.0, 100.0, "EUR", 1.0),
                ("TXN002", "20240102", "CUST002", "PROD002", "SELL", "SETTLED", "BRANCH", 550.0,  500.0,  50.0,  0.0, 5.0,  100.0, "EUR", 1.0),
                ("TXN003", "20240103", "CUST001", "PROD003", "BUY",  "PENDING", "API",    220.0,  200.0,  20.0,  0.0, 2.0,  100.0, "USD", 1.08),
            ],
            schema=[
                "TXN_ID", "TXN_DT", "CUST_ID", "PROD_ID",
                "TXN_TYPE_CD", "STATUS_CD", "CHANNEL_CD",
                "GROSS_AMT", "NET_AMT", "FEE_AMT", "TAX_AMT",
                "QTY", "UNIT_PRICE", "CURRENCY_CD", "FX_RATE_EUR"
            ]
        )
    ...
    def test_row_count(self, spark, transformer):
        ...
        result = transformer.transform_fact_transactions(
            raw_df=raw,
            dim_time=dim_ti, dim_customer=dim_cu,
            dim_product=dim_pr, dim_city=dim_ci,
            batch_id="TEST_001"
        )
    ...
    def test_time_sk_computed(self, spark, transformer):
        ...
        result = transformer.transform_fact_transactions(
            raw_df=raw, dim_time=dim_ti, dim_customer=dim_cu,
            dim_product=dim_pr, dim_city=dim_ci, batch_id="TEST_002"
        )
    ...
    def test_amount_eur_computed(self, spark, transformer):
        ...
        result = transformer.transform_fact_transactions(
            raw_df=raw, dim_time=dim_ti, dim_customer=dim_cu,
            dim_product=dim_pr, dim_city=dim_ci, batch_id="TEST_003"
        )
    ...
    def test_deduplication_keeps_latest(self, spark, transformer):
        ...
        raw = spark.createDataFrame(
            [
                ("TXN001", "20240101", "CUST001", "PROD001", "BUY", "SETTLED", "ONLINE", 1100.0, 1000.0, 100.0, 0.0, 10.0, 100.0, "EUR", 1.0),
                ("TXN001", "20240102", "CUST001", "PROD001", "BUY", "SETTLED", "ONLINE", 1100.0, 1000.0, 100.0, 0.0, 10.0, 100.0, "EUR", 1.0),
            ],
            schema=[
                "TXN_ID", "TXN_DT", "CUST_ID", "PROD_ID",
                "TXN_TYPE_CD", "STATUS_CD", "CHANNEL_CD",
        ...
        result = transformer.transform_fact_transactions(
            raw_df=raw, dim_time=dim_ti, dim_customer=dim_cu,
            dim_product=dim_pr, dim_city=dim_ci, batch_id="TEST_004"
        )
        assert result.count() == 1
    ...
    def test_batch_id_set(self, spark, transformer):
        ...
        result = transformer.transform_fact_transactions(
            raw_df=raw, dim_time=dim_ti, dim_customer=dim_cu,
            dim_product=dim_pr, dim_city=dim_ci, batch_id="BATCH_XYZ"
        )
    ...
    def test_surrogate_keys_resolved(self, spark, transformer):
        ...
        result = transformer.transform_fact_transactions(
            raw_df=raw, dim_time=dim_ti, dim_customer=dim_cu,
            dim_product=dim_pr, dim_city=dim_ci, batch_id="TEST_005"
        )
