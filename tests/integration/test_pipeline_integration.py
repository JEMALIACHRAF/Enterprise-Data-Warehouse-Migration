...
def read_oracle(spark, table: str, num_partitions: int = 2):
    ...
    return (
        ...
        .option("url", jdbc_url)
        .option(
            "dbtable", f"(SELECT t.*, ORA_HASH(ROWID,{num_partitions}) PART_ID FROM {table} t) Q"
        )
...
def read_snowflake(spark, table: str):
    return spark.read.format("net.snowflake.spark.snowflake").options(**sf_options(table)).load()
...
class TestEndToEndReconciliation:
    def test_fact_txn_count_matches(self, spark_real):
        ...
        df = read_oracle(spark_real, "FACT_TXN").select(
            ...
            F.round(F.col("NET_AMT").cast("float") * F.col("FX_RATE_EUR").cast("float"), 4).alias(
                "amount_eur"
            ),
